    ...
    >
      <div class="flex flex-col h-full justify-between text-xs pt-2">
        <div class="space-y-5">
          ...
          <!-- Quick Links -->
          <div class="space-y-2">
            <span class="text-[9px] font-bold text-slate-400 uppercase tracking-widest px-1">Akses Cepat</span>
            <button
              class="w-full px-3 py-2.5 rounded-xl text-[10px] font-bold bg-slate-50 text-slate-700 border border-slate-200 hover:bg-slate-100 transition-all flex items-center justify-between"
              @click="router.push({ name: 'SellerChatList' }); mobileMenuOpen = false"
            >
              <span class="flex items-center gap-1.5"><i class="pi pi-comments text-primary/70"></i> Pesan Chat</span>
              <span v-if="chatStore.unreadCount > 0" class="bg-primary text-white text-[9px] font-bold px-2 py-0.5 rounded-full">{{ chatStore.unreadCount }} baru</span>
            </button>
          </div>
        </div>
          </div>
